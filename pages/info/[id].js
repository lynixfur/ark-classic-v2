...
export async function getServerSideProps({ params }) {
  const id = params.id;
   const res = await fetch(`https://arkclassic.com/api/wiki/${id}`);
   const list_res = await fetch(`https://arkclassic.com/api/wiki/page_list`);
...
function ServerList({wiki_data, page_list}) {
  return (
    <>
      <Head>
      <title>ARK Classic - Information</title>
